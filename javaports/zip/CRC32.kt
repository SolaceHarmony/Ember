 ...
 *
 * This code is distributed in the hoape that it will be useful, but WITHOUT
...
class CRC32 : Checksum {
    ...
    override fun update(buffer: ByteBuffer) {
        ...
        if (buffer.isDirect) {
            ...
            try {
                crc = updateByteBuffer(crc, (buffer as DirectBuffer).address(), pos, rem)
            } finally {
    ...
    @IntrinsicCandidate
    private external fun update(crc: Int, b: Int): Int

    private fun updateBytes(crc: Int, b: ByteArray, off: Int, len: Int): Int {
        updateBytesCheck(b, off, len)
        return updateBytes0(crc, b, off, len)
    }

    @IntrinsicCandidate
    private external fun updateBytes0(crc: Int, b: ByteArray, off: Int, len: Int): Int

    private fun updateBytesCheck(b: ByteArray, off: Int, len: Int) {
        if (len <= 0) {
            return  // not an error because updateBytesImpl won't execute if len <= 0
        }

        Objects.requireNonNull(b)
        Preconditions.checkIndex(off, b.size, Preconditions.AIOOBE_FORMATTER)
        Preconditions.checkIndex(off + len - 1, b.size, Preconditions.AIOOBE_FORMATTER)
    }

    private fun updateByteBuffer(
        alder: Int, addr: Long,
        off: Int, len: Int
    ): Int {
    ...
    @IntrinsicCandidate
    private external fun updateByteBuffer0(
        alder: Int, addr: Long,
        off: Int, len: Int
    ): Int

    private fun updateByteBufferCheck(addr: Long) {
